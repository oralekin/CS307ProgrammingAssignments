...
#define PAGECOUNT (UINT16_MAX / (PAGE_SIZE / 2))
...
uint32_t getBitmap();
void setBitmap(uint32_t bm);
inline static uint16_t *pcb_address(uint16_t pid);
static void inline context_switch();

uint32_t getBitmap() {
	return mem[OS_FREE_BITMAP] | (mem[OS_FREE_BITMAP + 1] << 16);
}

void setBitmap(uint32_t bm) {
	mem[OS_FREE_BITMAP] = bm & 0x0000ffff;
	mem[OS_FREE_BITMAP + 1] = bm >> 16;
	return;
}
...
int createProc(char *fname, char *hname) {
	...
	if (pcb_address(mem[Proc_Count + 1])	// (address of) end of next pcb + 1
			>																	// is after of
...
uint16_t allocMem(uint16_t ptbr, uint16_t vpn, uint16_t read, uint16_t write) {
	...
	setBitmap(bitmap ^ (1 << pfn));

	fprintf(
		stdout, "allocated some memory for ptbr(0x%x), vpn(0x%x)\n", ptbr, vpn
	);
	fprintf(stdout, "\tfound space at page: ");
	fprintf_mem(stdout, mem, pfn * PAGE_SIZE / 2, (pfn * PAGE_SIZE / 2) + 1);
...
int freeMem(uint16_t vpn, uint16_t ptbr) {
	...
	if ((pte & VALID_PTE) == 0) return 0;	 // not allocated
		...
		// set bit on bitmap
		setBitmap(getBitmap() | 1 << ((pte >> 11) & 0x001f));
...
static void inline context_switch() {
	...
	for (int i = 1; i <= proc_count; i++) {
		...
		uint16_t *pcb = pcb_address(next_pid);
		if (pcb[PID_PCB] != 0xffff) {						// found unterminated process
			if (next_pid == current_pid) return;	// fast path if process not changed.

			// save pc, ptbr.
			// this program should continue from the next instruction!
			pcb[PC_PCB] = reg[RPC] + 1;
			pcb[PTBR_PCB] = reg[PTBR];
...
static inline void tyld() {
	fprintf(stdout, "We are switching from process %d to ", mem[Cur_Proc_ID]);
	context_switch();
	fprintf(stdout, "%d.\n", mem[Cur_Proc_ID]);
	return;
}
...
static inline uint16_t mr(uint16_t address) {
	...
	// reserved space
	if (vpn < 3) {
		fprintf(stdout, "Segmentation fault.\addr=");
		fprintf_binary(stdout, address);
		fprintf(stdout, "\n");
		exit(1);
	}
	...
	return mem[make_physical_address(pte, offset)];
}
...
static inline void mw(uint16_t address, uint16_t val) {
	...
	mem[make_physical_address(pte, offset)] = val;
	return;
}
